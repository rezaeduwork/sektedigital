<div>
  <button data-toggle="modal" data-target="#show-deleted-modal" class="btn btn-secondary btn-sm">Deleted Category ({{\App\Models\CategoryProduct::onlyTrashed()->count()}})</button>
          ...
          <table class="table table-stripped mb-2">
            ...
            <tbody>
              ...
              <tr wire:key="{{'table-deleted-modal'.$row->id}}">
                ...
                <td>
                  <livewire:admin.category.restore :id="$row->id" :key="'restore-'.$row->id" />
                </td>
              </tr>
              @endforeach
            </tbody>
