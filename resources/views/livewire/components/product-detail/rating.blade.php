      ...
      <div class="w-full">
        <div class="flex flex-col gap-2 sm:gap-6 gap-2 sm:mb-6">
          <div class="flex flex-col gap-2">
            <div class="lg:flex items-center gap-6">
              ...
              <small class="text-yellow-300 inline-flex items-center">
                ...
                @if($item <= $total_rating_star)
                  @if($total_rating_star != floor($total_rating_star))
              ...
              <span> {{round($total_rating_star,1)}} dari 5</span>
              <span>{{$total_rating}} orang memberi rating</span>
            </div>
          </div>
          <div class="flex flex-col gap-3">
            ...
            @foreach (range(1,5) as $item)
            @php
            $ratingNumber = $product->ratings()->where('rating', $item)->count();
            @endphp
            <div class="flex items-center gap-4">
              ...
              <div class="w-full bg-gray-200 rounded-full h-1.5">
                <div class="bg-yellow-300 h-1.5 rounded-full" style="width: {{$total_rating > 0 ? ($ratingNumber / $total_rating) * 100: 0}}%"></div>
              </div>
              <span class="text-gray-500">{{$total_rating > 0 ? ($ratingNumber / $total_rating) * 100: 0}}</span>
            </div>
            @endforeach
          </div>
          {{-- <div class="flex flex-col gap-4">
            <div class="flex flex-col">
              <h3 class="text-md font-semibold">Review this product</h3>
