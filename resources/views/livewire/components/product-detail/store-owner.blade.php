<div class="flex items-center justify-center max-sm:mt-6 mb-6">
  <div class="flex max-sm:flex-col items-center max-sm:space-y-7 sm:space-x-7 w-full">
    <div class="flex max-sm:flex-col items-center sm:space-x-7 shrink-0">
      <img src="{{storeProfile($store)}}" alt="" srcset="" class="size-[48px] sm:size-[78px] shrink-0" />
      <div class="shrink-0">
        <div class="text-lg text-gray-700 font-semibold max-sm:text-center">{{$store->name}}</div>
        <div class="max-sm:text-center">Aktif {{\Carbon\Carbon::parse($store->updated_at)->diffForHumans()}}</div>
        <button class="bg-violet-50 border border-primary text-primary px-4 py-2 mt-4 sm:mt-2 flex items-center space-x-2 font-semibold max-sm:justify-center">
    ...
    <div class="w-full grid grid-cols-1 sm:grid-cols-2 lg:gap-6">
      <div class="flex items-center justify-between sm:w-full border-b pb-2">
        <div>Produk</div>
