      ...
      @if (!auth()->user()->store)
        ...
        <div class="mb-5">
          <div class="space-y-2" x-data>
            ...
            @endif
            <input wire:model.live="photo" class="hidden" type="file" accept="image/*" id=""
              x-ref="photo" />
            <button type="button" class="px-4 py-2 border hover:bg-primary hover:text-white"
            ...
            <div x-data="{ uploading: false, progress: 0 }" x-on:livewire-upload-start="uploading = true"
              ...
              x-on:livewire-upload-progress="progress = $event.detail.progress" x-show="uploading"
          ...
          @error('photo')
            <span class="text-primary font-semibold">{{ $message }}</span>
          @enderror
        ...
        <button type="button" wire:click="store"
          class="text-white bg-primary hover:bg-primary focus:ring-4 focus:ring-violet-300 font-semibold rounded-lg text-sm px-5 py-2.5 me-2 mb-2">
          Simpan
        </button>
