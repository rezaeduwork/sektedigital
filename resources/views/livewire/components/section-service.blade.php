<div class="space-y-6">
  ...
  <template x-if="false">
    ...
    <div class="background-image bg-gradient-to-b from-violet-900 to-violet-700 text-white p-4 space-y-2 rounded-md" style="background: url('{{url('assets/images/bg-violet.jpeg')}}')">
      ...
      @endphp
      <div class="relative">
        <div class="flex items-center space-x-2 w-full overflow-x-auto overflow-y-hidden max-w-full hidden-scroll pb-2" x-ref="tabscontainer">
          @foreach ($list as $row)
        ...
        @click="$refs.tabscontainer.scrollLeft += 50"
        x-show="$refs.tabscontainer && $refs.tabscontainer.scrollLeft < ($refs.tabscontainer.scrollWidth - $refs.tabscontainer.clientWidth)"
